...
if ( dim(red.res)[1] == 0 && dim(red.res)[2] == 0 ) {
    ...
    output_parameters_table <- data.frame(
        variable=c('malt_extract_mode','malt_extract_output_folder','paired_end_mode','node_list','reads_threshold','damage_cutoff','read_distribution_cutoff','default_edit_distance_ratio','ancient_edit_distance_ratio','firm_cutoff_for_output'),
        values=c(mltexmd,opt$rmaex.out.fld,toString(paired_end_mode),opt$node,reads_threshold,dmgcutoff,readdistcutoff,defratio,ancratio,firm)
    )

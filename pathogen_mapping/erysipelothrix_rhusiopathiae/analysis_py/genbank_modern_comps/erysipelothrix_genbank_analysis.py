...
# remove suppressed records from NCBI: (see accessions_to_suppress.txt)
...
called_100_modern_ingroup_outgroup=np.where(np.sum(calls[:,((ingroup_modern_bool | outgroup_bool ))]<4,axis=1) >= (np.sum((ingroup_modern_bool | outgroup_bool ))))[0]
...
goodpos=np.intersect1d(candpos_modern_ldo,called_100_modern_ingroup_outgroup)
...
for s_indx in np.where(ancient_bool)[0]:
    ...
    if ancient_sample_to_subset_to == 'EASI241':
        ...
        candpos_modern_100del_called_ancient=np.intersect1d(called_100_ancient_sample,called_100_modern_ingroup_outgroup)

...
def read_latest_cutoff_vals(main_workingdir, refgenome):
    ## find lates cutoff val file saved and read in values

    ## identify saved latest file by timestamp in name
    latest_cutoff_file = glob.glob(main_workingdir + refgenome + '/*/*_cutoff_values.csv')
...
def generate_tree(calls_for_tree,treeSampleNamesLong,sampleNamesDnapars,refgenome,filetag,buildTree=False,writeDnaparsAlignment=False,coloring_pattern=''):
    ...
    if buildTree=='PS':      
        ...
        if coloring_pattern:
            ...
            unique_patterns = sort(unique(patterns))
            ...
            cmap_subset = matplotlib.colors.LinearSegmentedColormap.from_list('Custom cmap', cmap(np.linspace(minval, maxval, num_colors)), cmap.N)  ## subset cmap to remove unreadable bright colors 
            color = {}
            ...
            ## read tree again 
            f = open(timestamp+".tree", 'r').readlines()
            ## open outfile
            fo = open(timestamp+"_colored.tree", 'w')
...
def build_table_for_tree_labeling(p_chr_table,treeSampleNamesLong,calls_for_tree,patient="",indels=False):
    ...
    if patient != "":
        ...
        if not indels:
            subprocess.run(["rm -fr tree_counting/snps ; mkdir tree_counting/snps " ],shell=True)
        else:
            subprocess.run(["rm -fr tree_counting/indels ; mkdir tree_counting/indels " ],shell=True)
    # build table    
    if indels:
...
"""
TDL: countMutations.py functions below until countMutations()
"""
            ...
            if tree[i]=='(' and tree[i+4]==')' and tree[i+1]==tree[i+3]: #we have pair of two closest related strains, simplify
                ...
                if tree[i-2] in ACGT and tree[i+5]==')':
                    ...
                    if anc!=tree[i+1] and anc!=tree[i+3]:  #rare, but has happened, when tree is... (A,(C,T)).
                        if len(tree) > i+9: ## 20220331 Martin: Added len(tree) > i+9 as this might break in very rare cases (note: len() is not 0-based!)
                            if tree[i+9] in ACGT:
                                anc=tree[i+9]
...
def mutationtypes(tree, samples_basecalls_csv, outgroup_name, count_mutations_for_name, color_lca, label_lca, indels):
    """
    ...
    for i, line in enumerate(f[1:]):
        ...
        lca=l[lca_index]
        #count mutations
        newtree_nexus_snp_colored = annotateSNP(tree, samples_basecalls_csv, chromosome, pos,outgroup_name, color_lca, label_lca,indels)
        pos_for_output=str(int(pos)+1)
        if count_mutations_for_name:
            a, mutlist= mutation_count(newtree_nexus_snp_colored, lca, pos)
        #if a==0:
            #print('NO MUTS:')
            #print(chromosome, pos)
        #save trees
            if a != 1:
                f1=open(str(a)+'_'+chromosome+'_'+pos_for_output+'.tree','w')
            else:
                f1=open('1_'+chromosome+'_'+pos_for_output+'.tree','w')
        else:
...
def annotateSNP(tree, bascalls_per_sample_csv, chromosome, pos, reference_name_in_tree, color_lca, label_lca,indels):
    """
    creates temptree.txt tsv file w/ basecall for each sample, then calls nameswap to generate new tree file with basecall as color labels

    changelog:
    6.2022 IL: added docstring, removed limit to sample numbers and naming convention required to add sample names to temptree.txt, renamed dict --> bascalls_per_sample_csv for clarity, added parsing for nameswap
    """
    ...
    for line in f:
        ...
        if l[0]==chromosome and l[1]==pos:
            ...
            break
    fo.close()
    newtree_nexus_snp_colored = nameswap(tree,'temptree.txt',reference_name_in_tree, color_lca, label_lca,indels,chromosome,pos)
    return newtree_nexus_snp_colored

def nameswap(tree, dictionary, reference_name_in_tree, color_lca, label_lca, indels,chromosome,pos):
    """
    ...
    # TODO: check if numstrains calculation is OK?
...
def countMutations(tree,samples_basecalls_csv,outgroup_name_for_lca="", count_mutations_for_name=True, color_lca=False, label_lca=True, reroot_tree=False, indels=False):
    """
    Main function for generating trees for every SNP, calls mutationtypes and sets font. Set values for downstream in this function. 
    ...
    print("Note: Outputting name of tree with number of mutational events for this mutation (position) is currently", count_mutations_for_name, "... set count_mutations_for_name to True/False to modify")
    print('Note: Uses first strain as LCA, unless outgroup_name_for_lca provided as a string.')
    if outgroup_name_for_lca != "": print("Outgroup name provided, will attempt to use sample", outgroup_name_for_lca, "as outgroup.")
    rc('font', **{'sans-serif':'Arial'})
    if reroot_tree:
        print("Attempting to reroot tree, based on provided outgroup_name_for_lca",outgroup_name_for_lca)
        ## check outgroup provided for rerooting
        if outgroup_name_for_lca == "":
            print("No outgroup defined for rerooting, skipping rerooting.")
        else:
            input_tree=Phylo.read(tree, "newick")
            input_tree.root_with_outgroup(outgroup_name_for_lca)
            tree=tree.replace("_latest","_latest_rerooted_"+outgroup_name_for_lca)            
            Phylo.write(input_tree, tree, "newick")
            print()
            print(f"Successfully rerooted tree to filename",tree)
    mutationtypes(tree,samples_basecalls_csv,outgroup_name_for_lca, count_mutations_for_name, color_lca, label_lca, indels)
    if indels:
...
def parallel_evo_module(goodpos2use,contig_positions,annotation_mutations, annotation_genes, params_dict ,plot=True,ortholog=True,plot_title=None):
    '''
    Changelog:
    ...
    09/2022 IL: added dynamic xtick plotting capacity
    ...
    if ortholog:
        ...
        res_cand_nummut = np.vstack((locustags_cand,mut_cand_tally,prob_cand_nummut)).T
        print('Locustag NumMuts ProbSim')
        print(res_cand_nummut)
        with open("parallel_evo_module_results" + params_dict['subjectID'] + ".txt", "a") as myfile:
            myfile.write('Locustag NumMuts ProbSim'  + "\n")
            np.savetxt(myfile,  res_cand_nummut,fmt="%s")
    # =============================================================================
    # dN/dS calculation
    # =============================================================================
    ...
    if plot:
        ...
        if max_on_x_axis>100:
            ...
            step=10
        elif max_on_x_axis>25:
            step=5
        else:
            step=1
        plt.xlim(0,max(len(mut_cand_tally),max(np.unique(expectedNumberGenesMultipleMutations)))+1)
        xticks(np.arange(0,max(len(mut_cand_tally),max(np.unique(expectedNumberGenesMultipleMutations)))+step,step))
        ...
        if 'output_name' in params_dict:
            ...
            f.savefig('pdf/adaptive_evo/' + params_dict['output_name'] + "_" + params_dict['subjectID'] + ".pdf")
            plt.close()
            print('Plotted: pdf/adaptive_evo/' + params_dict['output_name'] + "_" + params_dict['subjectID'] + ".pdf")
        else:
            subprocess.run(["mkdir -p pdf/adaptive_evo/ "],shell=True)
...
def infer_non_snp_events(mutantAF,p,distance_for_nonsnp=500,fraction_covariance_nonsnp=0.98, print_bool=True):
